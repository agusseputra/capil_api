...
<div class="media-left">
	<h2 class=" ">
		<b >Data Mahasiswa</b>  
        <small class="display-block text-white  no-margin">Modul ini akan menampilkan data mahasiswa terakhir di 
...
<div class="col-lg-12" id="chart_div" style=" height: 400px;"> </div>
<div class="col-md-12"><hr /></div>
<div class=" col-md-12 tab_isi">
<?php $this->load->view('front/table_status_mahasiswa'); ?>
</div>
</div>
...
$(".select-clear option[value='all']").hide();
    $('.btn_cari').on('click', function () {
        ...
        $.ajax({ 
                    url		: "<?=site_url('data/mahasiswa/?ajax&');?>semester="+semester+"&fakultas="+fakultas+"&jurusan="+jurusan,
        			 success: function(response){
                        $('.tab_isi').html(response);
                       history.replaceState({pg:1}, "pg home", "?semester="+semester+"&fakultas="+fakultas+"&jurusan="+jurusan);
                       l.stop();
                       }
                });
    });	
