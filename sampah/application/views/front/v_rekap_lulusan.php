...
<div class="media-left">
	<h2 class=" ">
		<b >Statistik Mahasiswa Lulusan berdasarkan Periode Lulus</b>  
        <small class="display-block text-white  no-margin">Modul ini akan menampilkan data mahasiswa berdasarkan status terakhir di 
        Universitas Pendidikan Ganesha. Terdapat 4 pilihan untuk melakukan filter data yang diinginkan yaitu tahun,angkatan, fakultas dan prodi. 
...
<?php $this->load->view('front/table_lulusan'); ?>
</div>
</div>
...
$(".select-clear option[value='all']").hide();
    $('.btn_cari').on('click', function () {
        ...
        var tahun=$('.tahun').val();
         $.ajax({ 
                    url		: "<?=site_url('data/lulusan/?ajax&');?>tahun="+tahun+"&fakultas="+fakultas+"&jurusan="+jurusan+"&angkatan="+angkatan,
        			 success: function(response){
                        $('.tab_isi').html(response);
                       history.replaceState({pg:1}, "pg home", "?tahun="+tahun+"&fakultas="+fakultas+"&jurusan="+jurusan+"&angkatan="+angkatan);
                        l.stop();
                       }
                });
    });	
